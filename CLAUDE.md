...
- 26: AI Coach generates the meal plan — `generateMealPlan()` uses Opus 4.7 with tool use to produce a structured plan, validates each ingredient name against `foodPrefs.excluded` server-side (rejects on match). Uses the user's actual logged-foods frequency (last 14d) so the plan reuses what they already eat. New routes `/api/coach/regenerate-plan` (JWT, 1h rate limit). Sunday 09:00 UK cron auto-regenerates when `refreshCadence === weekly-sunday` (6d guard) or `biweekly` (13d guard). "Regenerate plan" button on Food page (replaces empty state with a Generate button when no plan exists). New `lastMealPlanRegenAt` state field.
